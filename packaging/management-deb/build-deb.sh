...
PACKAGE_NAME="linux-edge-inspection-management"

# 環境変数が指定されていない場合は0.2.0を使用します。
...
# packaging/management-debから2階層上をRepositoryルートとします。
REPOSITORY_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"

# publish対象となるManagement Hostプロジェクトです。
PROJECT_PATH="${REPOSITORY_ROOT}/src/Management/LinuxEdgeInspection.Management/LinuxEdgeInspection.Management.csproj"

# publish後に実行するアプリケーションDLLです。
APPLICATION_DLL="LinuxEdgeInspection.Management.dll"
...
# Blazor Host本体、wwwroot、Plugin DLLなど、
# publish成果物をまとめて/opt配下へ配置します。
cp -a \
    "${PUBLISH_DIR}/." \
    "${WORK_DIR}/opt/${PACKAGE_NAME}/"
...
# /usr/bin/linux-edge-inspection-management を作成します。
#
# 手動確認時は、
# linux-edge-inspection-management
# でManagement Hostを起動できます。
cat > "${WORK_DIR}/usr/bin/${PACKAGE_NAME}" <<'WRAPPER'
#!/usr/bin/env bash

exec /usr/bin/dotnet \
    /opt/linux-edge-inspection-management/LinuxEdgeInspection.Management.dll \
    "$@"
WRAPPER
...
Maintainer: mono-tec
Description: Management web application for Linux Edge Inspection
 A framework-dependent ASP.NET Core Blazor application that provides
 the management UI and plugin-based monitoring interfaces.
CONTROL
