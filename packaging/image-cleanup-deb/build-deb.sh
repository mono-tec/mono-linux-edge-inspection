...
PACKAGE_NAME="linux-edge-inspection-image-cleanup"
...
# packaging/image-cleanup-debから2階層上をRepositoryルートとします。
REPOSITORY_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"

# publish対象となるImageCleanupプロジェクトです。
PROJECT_PATH="${REPOSITORY_ROOT}/src/LinuxEdgeInspection.ImageCleanup/LinuxEdgeInspection.ImageCleanup.csproj"

# publish後に実行するアプリケーションDLLです。
APPLICATION_DLL="LinuxEdgeInspection.ImageCleanup.dll"

# パッケージへ含めるsystemd Service Unitファイルです。
SERVICE_FILE="${PACKAGE_NAME}.service"

# パッケージへ含めるsystemd Timer Unitファイルです。
TIMER_FILE="${PACKAGE_NAME}.timer"
...
# systemd Service Unitファイルをパッケージへ配置します。
cp \
  "${SCRIPT_DIR}/${SERVICE_FILE}" \
  "${WORK_DIR}/lib/systemd/system/${SERVICE_FILE}"

# systemd Timer Unitファイルをパッケージへ配置します。
cp \
  "${SCRIPT_DIR}/${TIMER_FILE}" \
  "${WORK_DIR}/lib/systemd/system/${TIMER_FILE}"
...
# /usr/bin/linux-edge-inspection-image-cleanupを作成します。
#
...
exec /usr/bin/dotnet \
  /opt/linux-edge-inspection-image-cleanup/LinuxEdgeInspection.ImageCleanup.dll \
  "$@"
WRAPPER
...
# 依存パッケージなどを定義します。
#
# ImageCleanupは.NET 10 Runtime上で動作する
# framework-dependentなone-shotアプリケーションです。
#
# 撮像画像の保持期限を確認し、
# 古くなったjpg/jpegファイルを削除します。
...
Maintainer: mono-tec
Description: Image cleanup service for Linux Edge Inspection
 A framework-dependent .NET application that removes expired capture
 images from the Linux Edge Inspection data directory.
CONTROL
...
#
# ImageCleanupは、他のLinux Edge Inspectionサービスと同じ
# linux-edge-inspectionユーザーで実行します。
#
# timerの自動有効化や自動起動はここでは行いません。
# install.shまたは実機検証時に明示的に有効化します。
...
# 新しく配置されたService / Timer Unitをsystemdへ認識させます。
...
#
# one-shot Serviceが実行中の場合も停止します。
#
# ServiceやTimerが未登録または未起動でも、
...
#
# ImageCleanupパッケージ単体の削除では、
# /var/lib/linux-edge-inspection配下の撮像画像は削除しません。
#
# 撮像画像はLinux Edge Inspection全体で利用する共通データであり、
# ImageCleanupパッケージだけの所有物ではないためです。
#
# linux-edge-inspectionユーザーとグループも、
# 他のLinux Edge Inspectionパッケージで共通使用するため
# このパッケージの削除時には削除しません。
