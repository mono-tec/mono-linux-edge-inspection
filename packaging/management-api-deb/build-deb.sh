...
PACKAGE_NAME="linux-edge-inspection-management-api"
...
# packaging/management-api-debから2階層上をRepositoryルートとします。
REPOSITORY_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"

# publish対象となるManagement APIプロジェクトです。
PROJECT_PATH="${REPOSITORY_ROOT}/src/Management/LinuxEdgeInspection.Management.Api/LinuxEdgeInspection.Management.Api.csproj"

# publish後に実行するアプリケーションDLLです。
APPLICATION_DLL="LinuxEdgeInspection.Management.Api.dll"
...
exec /usr/bin/dotnet \
    /opt/linux-edge-inspection-management-api/LinuxEdgeInspection.Management.Api.dll \
    "$@"
WRAPPER
...
# Management APIはASP.NET Core Webアプリケーションのため、
# ASP.NET Core Runtimeへ依存します。
#
# また、Unix Domain Socket経由でInspection Workerへ要求するため、
# 同一バージョンのInspection Workerへ依存させます。
...
Depends: aspnetcore-runtime-10.0, linux-edge-inspection-inspection-worker (= ${PACKAGE_VERSION})
Maintainer: mono-tec
Description: Management API for Linux Edge Inspection
 A framework-dependent ASP.NET Core API that provides local management
 endpoints and communicates with the inspection worker.
CONTROL
