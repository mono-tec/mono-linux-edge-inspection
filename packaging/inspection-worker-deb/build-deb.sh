...
PACKAGE_NAME="linux-edge-inspection-inspection-worker"
...
# packaging/inspection-worker-debから2階層上をRepositoryルートとします。
REPOSITORY_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"

# publish対象となるInspection Workerプロジェクトです。
PROJECT_PATH="${REPOSITORY_ROOT}/src/LinuxEdgeInspection.InspectionWorker/LinuxEdgeInspection.InspectionWorker.csproj"

# publish後に実行するアプリケーションDLLです。
APPLICATION_DLL="LinuxEdgeInspection.InspectionWorker.dll"
...
# /usr/bin/linux-edge-inspection-inspection-workerを作成します。
#
...
exec /usr/bin/dotnet \
  /opt/linux-edge-inspection-inspection-worker/LinuxEdgeInspection.InspectionWorker.dll \
  "$@"
WRAPPER
...
# 依存パッケージなどを定義します。
#
# Inspection WorkerはCapture Request Listenerへ
# Unix Domain Socket経由でCapture Requestを送信するため、
# 同一バージョンのCapture Request Listenerへ依存させます。
...
Depends: dotnet-runtime-10.0, linux-edge-inspection-capture-request-listener (= ${PACKAGE_VERSION})
Maintainer: mono-tec
Description: Inspection worker for Linux Edge Inspection
 A framework-dependent .NET worker service that coordinates inspection
 processing and communicates with the capture request listener.
CONTROL
